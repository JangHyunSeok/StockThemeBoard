...
graph LR
    subgraph "Docker Compose"
        A[nginx:latest<br/>Port 80/443]
        B[stocktheme-frontend<br/>Next.js Production]
        C[stocktheme-backend<br/>FastAPI + uvicorn]
        D[postgres:16-alpine<br/>Port 5432]
        E[redis:7-alpine<br/>Port 6379]
    end
...
#### 캐시 업데이트 전략
- **실시간 시세**: WebSocket 수신 시 즉시 업데이트
- **순위 데이터**: 30초마다 백그라운드 작업으로 재계산
- **토큰**: 만료 1시간 전 자동 갱신
