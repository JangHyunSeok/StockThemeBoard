...
- ✅ **GET /api/v1/rankings/volume-rank-by-theme**
  - KIS API에서 거래대금 상위 종목 조회
  - **멀티 마켓 지원**: `market` 파라미터 (KRX, NXT, ALL) 추가
  - **통합 시세(ALL) 지원**: 코스피/코스닥과 해외 거래소(NXT) 데이터를 병합하여 제공
  - KIS 업종명 기반 자동 분류 및 `SECTOR_OVERRIDE_MAP`을 통한 정밀 보정
  - 5초 캐싱 (실시간성 강화)
...
- 종목은 실시간 API에서 자동 수집
- KIS 업종명 기반 분류 + **섹터 오버라이드 맵**으로 정밀도 향상 (`src/backend/app/core/themes.py`)
- 국내(KRX)뿐만 아니라 해외/나스닥(NXT) 마켓 지원 및 통합 시세 제공
...
### 5. 데이터 자동 저장 및 공휴일 처리
- **Background Scheduler 도입**:
  - `APScheduler`를 사용하여 매일 **15:40**에 거래량 상위 종목 자동 저장.
  - 사용자가 접속하지 않아도 데이터가 누락되지 않도록 개선.
- **공휴일 대응 (Hybrid Mode)**:
  - `holidays` 라이브러리를 통해 주말 및 공휴일(설날, 추석 등)을 정확히 판별.
  - 휴장일에는 DB의 과거 순위 데이터를 기반으로 **실시간 시세(KIS API)**를 병렬 조회하여 제공.
  - **속도 최적화**: 병렬 호출(Chunking) + Redis Cache(5초) 적용.
